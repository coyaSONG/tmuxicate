# Requirements Archive: v1.1 Adaptive Coordination
...
- Adaptive routing shipped as an inspectable overlay on the existing deterministic routing baseline.
- Partial replanning shipped as a bounded blocker-resolution extension rather than a general autonomous re-planner.
- Remote and sandbox targets shipped as explicit durable placement metadata with local-runtime exclusion, not a built-in remote transport layer.
- Timeline and filtering shipped inside the existing `run show` flow rather than a separate reporting subsystem.
...
*Archived: 2026-04-11 after shipping v1.1 Adaptive Coordination*
