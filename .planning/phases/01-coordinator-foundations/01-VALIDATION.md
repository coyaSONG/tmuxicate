...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 01-01-01 | 01 | 1 | PLAN-01 | T-01-01 | Coordinator run creation validates inputs and emits durable artifacts only after canonical write succeeds | unit/integration-with-tempdir | `go test ./internal/session -run TestRunCreatesChildTasks -count=1` | ❌ W0 | ⬜ pending |
| 01-02-01 | 02 | 1 | PLAN-02 | T-01-02 | Child-task records persist all required fields with stable linkage and no transcript-only state | unit | `go test ./internal/session -run TestRunPersistsChildTaskFields -count=1` | ❌ W0 | ⬜ pending |
| 01-03-01 | 03 | 2 | PLAN-03 | T-01-03 | Restart/rebuild reconstructs coordinator run state from disk artifacts without hidden memory | unit/integration-with-tempdir | `go test ./internal/session -run TestRebuildRunGraphFromDisk -count=1` | ❌ W0 | ⬜ pending |
